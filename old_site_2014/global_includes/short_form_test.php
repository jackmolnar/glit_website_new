...
//if facebook profile link is given, include it in the email body
if (isset($fb_profile_link)){
	$body .= "<tr><td width='125' height='25'><strong>Facebook Profile Link:</strong></td><td> <a href='".$fb_profile_link."'> $fb_profile_link</a> </td></tr>";
}
//
...
//FB connect
 if($fb_user_id) {
      try {  
		...
		$date_of_birth = $user_profile['birthday'];
	...
	echo 'Save time! Get info from your Facebook page!<br>
	<a href="'.$loginUrl.'"><img src="_images/fb_connect.jpg" style="float:none;" /></a>';
        error_log($e->getType());
     ...
     $params = array(
  		'scope' => 'email, user_birthday, user_location',
  		//'redirect_uri' => 'http://www.glit.edu/request_more_information_fb.php'
	);

	$loginUrl = $facebook->getLoginUrl($params);

	echo '
	Save time! Get info from your Facebook page for the form below!<br>
	<a href="'.$loginUrl.'" onclick="track_fb_connect()"><img src="_images/fb_connect.jpg" style="float:none;" /></a>';
    }
//
//FB connect
